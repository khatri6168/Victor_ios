...
open class LoadingPlaceholderView: UIView {
    ...
    public struct GradientConfiguration {
        ...
        */
        ...
        public var isClourModeDark: Bool = UserDefaults.standard.object(forKey: "colourMode") as! String == "dark" ? true : false

        fileprivate mutating func setMainColor(_ color: UIColor) {
            backgroundColor = color.backgroundColor(brightness: 0.98, isMode: isClourModeDark)
            primaryColor = color.primaryColor(brightness: 0.95, isMode: isClourModeDark)
            secondaryColor = color.secondaryColor(brightness: 0.88, isMode: isClourModeDark)
        }
...
extension UIColor {
    ...
    fileprivate func backgroundColor(brightness: CGFloat, isMode : Bool) -> UIColor {
...
//        }
    ...
    fileprivate func primaryColor(brightness: CGFloat, isMode : Bool) -> UIColor {
        if isMode{
    ...
    fileprivate func secondaryColor(brightness: CGFloat, isMode : Bool) -> UIColor {
        if isMode{
            return UIColor(red: 30/255.0, green: 28/255.0, blue: 30/255.0, alpha: 1.0)
        }else{
            return UIColor(red: 245/255.0, green: 245/255.0, blue: 245/255.0, alpha: 1.0)
        }
