//
//  SaveArtistsViewController.swift
//  victor
//
...
class SaveArtistsViewController: UIViewController {
    ...
    var arrSaveArtistsList : [TrackModel] = []
    var arrSearchSaveArtistsList : [TrackModel] = []
    ...
    override func viewDidLoad() {
        ...
        //SET NODATA
        self.emptyDataView.noSaveArtists()
    ...
    func setTheView(){
        ...
        self.lblHeader.text = "\(str.strArtists)"
        if self.arrSaveArtistsList.count != 0{
            self.lblHeader.text = "\(self.arrSaveArtistsList.count) \(str.strArtists)"
        }
    }
    ...
    func setTheFont(){
        ...
        txtSearch.configureText(bgColour: .clear, textColor: setTextColour(), fontName: GlobalConstants.APP_FONT_Regular, fontSize: 14.0, text: "", placeholder: str.artistsSearchPlaceholder)
    ...
    @objc func textFieldDidChangeSearch() {
        ...
        //GET STORE LIST
        self.arrSearchSaveArtistsList = []
        self.arrSearchSaveArtistsList = self.arrSaveArtistsList.filter { Int((($0.name?.lowercased()) as NSString?)?.range(of: strSearch.lowercased()).location ?? 0) != NSNotFound}
        ...
        if txtSearch.text != ""{
            if arrSearchSaveArtistsList.count == 0{
...
extension SaveArtistsViewController:  UITextFieldDelegate {
...
extension SaveArtistsViewController : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    public func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if isLoading && UserDefaults.standard.user != nil{
            ...
            if txtSearch.text != ""{
                return arrSearchSaveArtistsList.count
            }
            else{
                return arrSaveArtistsList.count
            }
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if txtSearch.text != ""{
            objData = self.arrSearchSaveArtistsList[indexPath.row]
        }else{
            objData = self.arrSaveArtistsList[indexPath.row]
        }
        ...
        if let strImg = objData.image_small{
